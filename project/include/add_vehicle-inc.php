...
if($rownum>0){
    ...
    if($rownum1>0){
		echo "<script>alert('This vehicle already exist, please check the vehicle licence!');window.location='../add_vehicle.php'</script>";
        //header("Location:../add_vehicle.php?error=dataalreadyexists");
    }else{
        //add Vehicle table
        ...
        //header("Location:../add_vehicle.php?result=ownershipaddingsuccess!");
        //give notice "adding success"
    }
}else{
	echo "<script>alert('This person does not exist in the database, Please check the Owner!');window.location='../add_people.php'</script>";
    //header("Location:../add_people.php?error=nopersondataexists");
